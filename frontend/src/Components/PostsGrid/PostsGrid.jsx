...
import { FaEye, FaEdit, FaTrash, FaHeart, FaComment, FaTimes } from "react-icons/fa";

export default function PostsGrid({ posts, users, categories, onEdit, onDelete }) {
  const [selectedPost, setSelectedPost] = useState(null);

  const InforPost = posts.map((post) => {
    const user = users.find((u) => u.id === post.userId);
    const category = categories.find((cate) => cate.id === post.categoryId);
    return {
      ...post,
      postName : user ? user.name : "UserName Post",
      avatar: user ? user.avatar : "Avatar User",
      postCategory: category ? category.name : "Category Post"
    };
  });
